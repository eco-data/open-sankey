...
function add_links() {
	...
	// drag link_center	
	gg_links.filter(function(d){
		return nodes[d.source].orientation == "vertical" && nodes[d.target].orientation == "vertical";
	})
		.append("rect")
		.attr("id",function(d){
 		...
 		.attr("transform",function(d){
 			return drawCurve(d,true);
 		})
 		.call(d3.behavior.drag()
   			.origin(Object).on("drag", function() {
				drag_link_center(this);
			})
		);
}
...
function drag_node(dragged) {
    ...
    links.forEach(function(link,i){
        if (link.source == dragged.id.match(/\d+/)[0] || link.target == dragged.id.match(/\d+/)[0]) {
        	if (nodes[link.source].orientation == "vertical" && nodes[link.target].orientation == "vertical") {
				// Redraw link center
				d3.select('#link_center' + i).attr("transform", function(d) {
					return drawCurve(d,true);
				}); 
			}
            // Redraw link
...
// Drag link_center
function drag_link_center(dragged) {
    ...
    d.x_center = (new_x - nodes[d.source].x - default_node_size)  / link_x_length;
    if (d.x_center < 1/7 || d.x_center > 6/7) {
    	return;
    }
    links[d.id] = d;
    d3.select(dragged).attr("transform",function(){
    	return "translate(" + new_x + ", " + old_y_str;
    })
...
// DRAW LINK   
function drawCurve(d, link_center) {
	var source_orientation = nodes[d.source].orientation,
		target_orientation = nodes[d.target].orientation;
		
    var x0 = +d3.select('#gg_node' + d.source).attr("x"),
    	y0 = +d3.select('#gg_node' + d.source).attr("y"),
    	x5 = +d3.select('#gg_node' + d.target).attr("x"),
    	y5 = +d3.select('#gg_node' + d.target).attr("y");
	...
	var x1, y1, x2, y2, x3, y3, x4, y4,
		source_order = nodes[d.source].output_links.indexOf(d.id),
	...
	if (source_orientation == "vertical" && target_orientation == "vertical") {
		...
		if (link_center) {
			...
			var x_center_draw = (x5-x0) * d.x_center + x0 - default_node_size/2;
				y_center_draw = (y0+y5)/2 - default_node_size/2;
